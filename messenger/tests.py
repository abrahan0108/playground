...
class ThreadTestCase(TestCase):
    ...
    def test_filter_thread_by_users(self):
        self.thread.users.add(self.user1, self.user2)
        threads = Thread.objects.filter(users=self.user1).filter(
                                                          users=self.user2)
        self.assertEqual(self.thread, threads[0])

    def test_filter_none_existent_thread(self):
        threads = Thread.objects.filter(users=self.user1).filter(
                                                          users=self.user2)
    ...
    def test_add_message_to_thread(self):
        ...
        message2 = Message.objects.create(user=self.user2, content="Que pex?")
